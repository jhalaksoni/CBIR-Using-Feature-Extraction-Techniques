...
class VGGNet(VGG):
  def __init__(self, weights=None, model='vgg16', requires_grad=False, remove_fc=False, show_params=False):
    super().__init__(make_layers(cfg[model]))
    self.ranges = ranges[model]
    self.fc_ranges = ((0, 2), (2, 5), (5, 7))
    self.VGG_model  = 'vgg19'  # model type
    self.pick_layer = 'avg'    # extract feature of this layer
    self.d_type     = 'd1'     # distance type
    ...
    if weights is not None:
        self.load_state_dict(weights.get_model_weights())
    # if pretrained:
...
class VGGNetFeat(object):
  ...
  def make_samples(self, db, verbose=True):
    ...
    try:
      ...
      vgg_model = VGGNet(weights=models.VGG19_Weights.IMAGENET1K_V1, requires_grad=False, model=VGG_model)
      vgg_model.eval()
      if use_gpu:
      ...
      for d in data.itertuples():
        ...
        img = imageio.v2.imread(d_img, mode="RGB")
