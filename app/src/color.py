...
class Color(object):
  def __init__(self):
    # configs for histogram
    self.n_bin   = 12        # histogram bins
    self.n_slice = 3         # slice image
    self.h_type  = 'region'  # global or region
  ...
  def histogram(self, input, n_bin=n_bin, type=h_type, n_slice=n_slice, normalize=True):
    ...
    if isinstance(input, np.ndarray):  # examinate input type
      ...
      img = imageio.v2.imread(input, mode='RGB')  # Use imageio.v2.imread() instead of imageio.v2.imread()
...
if __name__ == "__main__":
  ...
  # test normalize
  hist = color.histogram(data.loc[0, 'img'], type='global')
